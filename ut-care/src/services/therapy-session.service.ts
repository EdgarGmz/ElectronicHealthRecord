...
export async function getTherapySessions(params: { page?: number; limit?: number; patientId?: string; therapistId?: string; psychologyRecordId?: string } = {}): Promise<TherapySessionsResponse> {
  const { page = 1, limit = 10, patientId, therapistId, psychologyRecordId } = params
