...
export function ConfirmEmailPage() {
  ...
  useEffect(() => {
    if (!token) {
      setStatus('error')
      setMessage(t('common.error') || 'Falta el token de confirmación en la URL.')
      return
    }

    const confirmEmail = async () => {
      try {
        const res = await api.post<{ success: boolean; message: string }>('/auth/confirm-email', { token })
        setStatus('success')
        setMessage(res.data.message || t('profilePage.updatedSuccess'))
      } catch (err: any) {
  ...
  const buttonLabel = user ? t('profilePage.goToProfile') : t('profilePage.goToLogin')
      ...
      <GlassCard className="w-full max-w-md text-center p-6 animate-fade-in">
        {status === 'loading' && (
          <div className="space-y-4">
            ...
            <h1 className="text-xl font-bold text-[var(--text-primary)]">
              {t('profilePage.confirmingEmailTitle')}
            </h1>
            <p className="text-[var(--text-secondary)]">
              {t('profilePage.confirmingEmailDesc')}
            </p>
          </div>
        )}
        ...
        {status === 'success' && (
          <div className="space-y-4">
            ...
            <h1 className="text-xl font-bold text-[var(--text-primary)]">
              {t('profilePage.confirmEmailSuccessTitle')}
            </h1>
            <p className="text-[var(--text-secondary)]">
              {message}
            </p>
        ...
        {status === 'error' && (
          <div className="space-y-4">
            ...
            <h1 className="text-xl font-bold text-[var(--text-primary)]">
              {t('profilePage.confirmEmailErrorTitle')}
            </h1>
            <p className="text-[var(--text-secondary)]">
              {message}
            </p>
