...
const ACTION_VALUES = ['LOGIN', 'LOGOUT', 'CREATE', 'READ', 'VIEW_RECORD', 'UPDATE', 'DELETE', 'EXPORT'] as const
...
function getActionRowClass(action: string): string {
  ...
  if (a === 'READ' || a === 'VIEW_RECORD' || a === 'EXPORT') return 'border-b border-[var(--border)] last:border-0 bg-[#8b5cf6]/12 dark:bg-[#8b5cf6]/20 border-l-4 border-l-[#8b5cf6]'
...
function getActionLabel(action: string, t: (key: string) => string): string {
  ...
  if (a === 'READ') return t('auditLogs.actionRead')
  if (a === 'VIEW_RECORD') return t('auditLogs.actionViewRecord')
  if (a === 'UPDATE') return t('auditLogs.actionUpdate')
  if (a === 'DELETE') return t('auditLogs.actionDelete')
  if (a === 'EXPORT') return t('auditLogs.actionExport')
  return action
}
...
export function AuditLogsPage() {
  ...
  const columns: DataTableColumn<AuditLog>[] = [
    ...
    { id: 'eventDetail', label: t('auditLogs.tableDetail'), getValue: (row) => row.eventDetail ?? '—' },
    ...
    <div className="container mx-auto px-4 py-6 space-y-6">
      ...
      <GlassCard>
        <DataTable
          ...
          filters={[
            { key: 'role', label: t('auditLogs.filterRole'), type: 'select', options: ['admin', ROLES.COORDINADOR_PSICOLOGIA, ROLES.COORDINADOR_ENFERMERIA, ROLES.PSICOLOGO, ROLES.ENFERMERO, 'patient'].map((v) => ({ value: v, label: t(`roles.${v}`) || v })) },
